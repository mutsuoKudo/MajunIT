$(function () {
        ...
        if(cnt) {
            $.ajax({
                ...
                if (res == "error") {
                  ...
                  alert("スキルシートは拡張子が(xls、xlsx、doc、docx、pdf)のみアップできます。")
    ...
    //詳細ページメールアドレス同期
    $('input[name=email]').bind('blur', function(e) {
        var input_email = $(this).val();
        $('input[name=email]').val(input_email);
    });

    //ファイルの添付 拡張子チェックJS
    $(".file").on('change','input' , function(){
        var name = $(this).val().split(".")[1];
        var tx = $(this).parent().parent().find("p:last");

       var filename = $(this).val().split('\\').pop();
        if(name) {
            if (name == 'xls' || name == 'xlsx' || name == 'pdf' || name == 'doc' || name == 'docx') {
